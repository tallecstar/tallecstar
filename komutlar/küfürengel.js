...
exports.run = async (client, message, args) => {
  ...
  if (!args[0])
    return message.channel.send(
      `Merhaba komutlar için lütfen !küfür-engel yardım yazınız!`
    );
  if (args[0] == "log") {
    ...
    const embed = new discord.MessageEmbed()
      ...
      .setDescription(
        `Küfür engel log kanalını <#${enginar.id}> olarak ayarladım.`
      );
  ...
  if (args[0] == "mesaj") {
    ...
    const embed = new discord.MessageEmbed()
      ...
      .setDescription(
        `Bundan sonra biri küfür eder ise <@${message.author.id}>, ${enginar} şeklinde cevap vereceğim!`
      )
      .setColor("RANDOM");
    return message.channel.send(embed);
  }
  ...
  if (args[0] == "aç") {
    ...
    if (!kontrol)
      return message.channel.send("Küfür engel log kanalı ayarlanmamış!");
    let enginar = db.fetch(`küfürengelmesaj_${message.guild.id}`);
    if (!enginar)
      return message.channel.send("Küfür engel mesajı ayarlanmamış");
    db.set(`küfürengel_${message.guild.id}`, "aktif");
    const embed = new discord.MessageEmbed()
      .setTitle("Küfür engel sistemi açıldı!")
      .setDescription(
        `<@${message.author.id}> bu sunucuda küfür engel sistemini aktifleştirdi!`
      )
      .setColor("RANDOM");
